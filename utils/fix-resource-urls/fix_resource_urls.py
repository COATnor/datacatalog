...
class Resource:
    ...
    Attributes:
        ...
        predecessor (str, optional): The ID of the preceding resource in the sequence.
            Defaults to `None`.
        head (bool, optional): Indicates if this resource is the "head" of the sequence.
            Defaults to `False`.
        successor (str, optional): The ID of the succeeding resource in the sequence.
            Defaults to `None`.
    ...
    def __str__(self) -> str:
        ...
        The string includes the resource's ID, name, predecessor ID (`predecessor`),
        whether it's the head, and the ID of its successor.
...
def find_resources(
    response: dict, resource_name: str, invalid_resource: dict, logger: Logger
) -> list:
    """
    ...
    Args:
        response (dict): The response dictionary from the CKAN API, which includes
            a `results` key containing details of the resources.
        resource_name (str): The name of the resource to find, used for logging or
            debugging purposes.
    ...
    Notes:
        - The function assumes a helper function `extract_resource_id(url: str)` is
          available to extract the resource ID from a URL.
        - The `Resource` class is expected to have a constructor with attributes for
          `id`, `name`, `url`, `predecessor`, and `head`.
    """
    resources = []
    if response.get("results"):
        for result in response["results"]:
            # extracting the ID of the resource which is pointed to
            prev_resource_id = extract_resource_id(
                result["url"]
            )  # this will return None for the faulty url/resource
        ...
        # BUG? if id of invalid resource is not in the list of resources, then add the
        # invalid resource to the list of resources
...
def find_faulty_and_last_resource(resources: list) -> tuple:
    """
    Identifies the faulty and last resources in a list of `Resource` objects and fixes
    their pointers.

    This function processes a list of `Resource` objects to determine relationships
    between them by setting the `successor` attribute. It then identifies:
    - The "faulty" resource, which is neither the head nor has a valid `predecessor`
      pointer.
    - The "last" resource in the sequence, which has no `successor`.

    Once identified, the function updates the `predecessor` pointer of the faulty
    resource to point to the last resource and sets the `successor` pointer of the
    last resource to point to the faulty resource.

    Args:
        resources (list): A list of `Resource` objects. Each `Resource` object must
            have the attributes `resource_id`, `predecessor`, `successor`, and `head`.
    ...
    faulty_res = next(
        (res for res in resources if res.predecessor is None and res.head is False), None
    )
...
def create_correct_url(response, faulty_res: Resource) -> str:
    ...
    # create the new URL
    new_url = (
        f"https://data.coat.no/dataset/{dataset_id}/resource/{resource_id}/download/{resource_name}"
    )
    return new_url
...
def get_invalid_resources(ckan: RemoteCKAN, dataset_id: str, logger: Logger) -> list:
    ...
    for resource in dataset.get("resources", []):
        ...
        try:
            r = requests.head(resource_url, timeout=10)
...
def update_resources(ckan: RemoteCKAN, invalid_resources: list, logger: Logger):
    ...
    for invalid_resource in tqdm(invalid_resources, desc="Updating resources"):
        ...
        if faulty_res is None or last_res is None:
            logger.error(
                f"Could not find faulty or last resource for {resource_name} "
                f"({invalid_resource.resource_id})"
            )
            continue
        response = ckan.action.resource_show(id=faulty_res.predecessor)
        fixed_url = create_correct_url(response, faulty_res)

        r = requests.head(fixed_url, timeout=10)
        try:
            if r.status_code == 302 or r.status_code == 200:
                updated_resource = {"id": faulty_res.resource_id, "url": fixed_url}
                _ = ckan.action.resource_patch(**updated_resource)
                logger.info(
                    f"Resource {resource_name} ({invalid_resource.resource_id}) updated "
                    f"successfully."
                )
                logger.info(f"New url: {fixed_url}")
            else:
                logger.error(
                    f"Resource {resource_name} ({invalid_resource.resource_id}) could not "
                    f"be fixed new url."
                )
...
def main():
    ...
    if dataset.get("private"):
        invalid_resources = get_invalid_resources(
            ckan=ckan, dataset_id=args.dataset_id, logger=logger
        )
