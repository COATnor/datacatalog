...
def lowercase_extension(value):
    return Path(value).stem + Path(value).suffix.lower()
...
def _resource_name_conflict_local(context, pkg_dict, name):
    for resource in pkg_dict["resources"]:
        if resource["name"].lower() == name.lower():
            raise Invalid("Resource with the same name in the same dataset already exists: " + name)


def resource_name_conflict(key, data, errors, context):
    name = data[(key[0], key[1], "name")]
    pkg_dict = toolkit.get_action("package_show")(context, {"id": context["package"].id})
    ...
    globally_unique = (
        config.get("ckanext.coat.resource_name_globally_unique", "false").lower() == "true"
    )
    if globally_unique:
